...
export async function updateLinearTokensByRsRefreshToken(
  rsRefreshToken: string,
  ...
  if (kv) {
    ...
    await Promise.all([
      kv.delete(`rs:access:${existing.rs_access_token}`),
  ...
  existing.linear = { ...newLinear };
  memRsByAccess.set(existing.rs_access_token, existing);
  memRsByRefresh.set(rsRefreshToken, existing);
  return existing;
}

export async function getLinearTokensByRsAccessToken(
  rsAccessToken?: string,
): Promise<LinearUserTokens | null> {
  if (!rsAccessToken) {
    return null;
  }
  const kv = getKV();
  if (kv) {
    ...
    return rec?.linear ?? null;
  }
  const mem = memRsByAccess.get(rsAccessToken);
  return mem?.linear ?? null;
}
